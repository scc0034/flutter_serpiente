...
class _TableroPageState extends State<TableroPage>
  ...
  int _cuentaAtrasMsg = -1;

  /// Controla el tiempo que se muestra el mensaje
      ...
      builder: (context, AsyncSnapshot snapshot) {
        ...
        if (snapshot.hasData) {
          ...
          if (_cuentaAtrasMsg < 0 && m.compareTo("") != 0) {
            ...
            /// Duración del mensaje hasta que se borra
          }
          return Container(
              height: 65,
              color: _colorR,
          ...
          Material(
            child: Container(
              ...
              child: IconButton(
                ...
                onPressed: () => _enableField
                    ? _enviarMsg(textEditingController.text)
                    : null,
              ),
            ),
            color: Colors.white,
    ...
    _timer = new Timer.periodic(
      ...
      (Timer _timer) => setState(() {
        ...
        setState(() async {
          ...
          if (_mapVarGame["lanzamiento"] == true &&
              _mapVarGame["mostrado"] == false) {
          ...
          if (finJuego) {
            _timer.cancel();
            _mostrarFinal(context, _mapVarGame["winner"].toString(),
                _mapVarGame["winnerImg"]);
          }
  ...
  Future<void> _putFicha(int index, String turno, bool endGame) async {
    ...
    if (endGame == false) {
      if (turno.compareTo("yellow") == 0 &&
          emailGoogle.compareTo(_mapVarGame["emailYellow"]) == 0) {
        ficha = "Y";
        _mapVarGame[index.toString()] = ficha;
        updated = true;
      }

      if (turno.compareTo("red") == 0 &&
          emailGoogle.compareTo(_mapVarGame["emailRed"]) == 0) {
        ficha = "R";
        _mapVarGame[index.toString()] = ficha;
        updated = true;
      }
      ...
      if (updated) {
        _mapVarGame["turno"].compareTo("yellow") == 0
      ...
      bool endGame = false;
      if (_controlFinHorizontal(index, ficha) ||
          _controlFinVertical(index, ficha) ||
          _controlFinDiagonal(index, ficha)) {
        endGame = true;
      }
  ...
  bool _controlFinHorizontal(int index, String ficha) {
    ...
    int fila;
    int inicioFila;
    Map<String, String> cadenaValidar = {"R": "RRRR", "Y": "YYYY"};
  ...
  bool _controlFinDiagonal(int index, String ficha) {
    ...
    index == 0
        ? fila = (0)
        : fila = (index ~/ _nFil); // ~/Parte entera de la division
    ...
    while (fila < _nFil) {
      ...
      print(
          "El nuevo valor del index = $indexII, con la cadena = $cadenaIISD, el valor de la fila = $fila");
      fila++;
      columna--;
    }
    columna++;
